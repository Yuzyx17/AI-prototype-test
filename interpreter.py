...
class LineGraph:
    ...
    def interpret_data(self):
        ...
        yy = regression_model.predict(x.reshape(-1, 1))
        r2 = r2_score(y, yy)
        ...
        if not local_maxima.empty:
            ...
            detailed_summary += ", ".join(f"({round(point['x'], 2)}, {round(point['y'], 2)})" for _, point in local_maxima.iterrows())
            detailed_summary += ".\n"

        if not local_minima.empty:
            detailed_summary += "There is a local valley at: "
            detailed_summary += ", ".join(f"({round(point['x'], 2)}, {round(point['y'], 2)})" for _, point in local_minima.iterrows())
            detailed_summary += ".\n"
    ...
    #     inputs = tokenizer.encode(self.summary, return_tensors="pt", max_length=1024, truncation=True)
    #     outputs = model.generate(inputs, max_length=150, num_return_sequences=5, num_beams=10, temperature=1.0)

    #     paraphrases = [tokenizer.decode(output, skip_special_tokens=True) for output in outputs]
    #     self.summary =  paraphrases
