...
class Canvas:
    ...
    def set_bb_vgg(self):
        ...
        self.bb_h = image_height-1 if self.bb_h < 0 or self.bb_h >= image_height else self.bb_h
        self.bb_y = 0 if self.bb_y < 0 or self.bb_y >= image_height else self.bb_y
        self.bb_w = image_width-1 if self.bb_w < 0 or self.bb_w >= image_width else self.bb_w
        self.bb_x = 0 if self.bb_x < 0 or self.bb_x >= image_width else self.bb_x

        self.bounding_rect = (self.bb_x, self.bb_y, self.bb_w, self.bb_h)
        self.canvas = self.image[self.bb_y:self.bb_h, self.bb_x:self.bb_w]
        print(self.bb_y, self.bb_h, self.bb_x, self.bb_w, self.image.shape)
